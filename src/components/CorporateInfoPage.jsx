...
const CorporateInfoPage = () => {
  ...
  return (
    <main className="min-h-screen bg-white font-sans selection:bg-rose-100 overflow-x-hidden">
      ...
      {/* --- HERO SECTION --- */}
      {/* Increased pt-36 and lg:pt-44 to prevent sticking to the fixed header */}
      <section className="relative pt-36 pb-16 lg:pt-44 lg:pb-24 px-10 lg:px-20">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-20">
            <div className="flex-1 space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              {/* Established Pill Moved Below Heading */}
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                <button 
                  ...
                  className="px-9 py-4 bg-rose-500 text-white rounded-full font-medium shadow-lg shadow-rose-200 flex items-center gap-2 cursor-pointer transition-all duration-300 hover:scale-105 hover:-translate-y-1 active:scale-95"
                >
                  Get Started 
                ...
                <button 
                  ...
                  className="px-9 py-4 bg-white text-slate-900 border border-slate-200 rounded-full font-medium flex items-center gap-2 cursor-pointer transition-all duration-300 hover:scale-105 hover:-translate-y-1 active:scale-95 hover:bg-slate-50"
                >
            ...
            <div className="flex-1 relative w-full animate-in fade-in zoom-in-95 duration-1000">
              <div className="relative aspect-square rounded-[2.5rem] overflow-hidden group shadow-2xl border border-slate-100">
                <img 
                  src="https://images.unsplash.com/photo-1529156069898-49953e39b3ac?q=80&w=2000&auto=format&fit=crop" 
                  alt="Connection"
                  className="object-cover w-full h-full grayscale-[15%] group-hover:grayscale-0 group-hover:scale-110 transition-all duration-1000"
                />
      ...
      {/* --- STATS SECTION --- */}
      <section id="stats-section" className="py-16 bg-white border-y border-slate-100 px-10 lg:px-20">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="group flex flex-col items-center gap-4 rounded-3xl border border-slate-50 bg-white p-8 text-center cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:scale-[1.02] hover:shadow-[0px_20px_40px_rgba(0,0,0,0.06)] hover:border-rose-100 hover:bg-rose-50/10 active:scale-95"
              >
                <div className={`p-3 rounded-2xl bg-slate-50 group-hover:bg-white transition-colors ${stat.color}`}>
                  {stat.icon}
                </div>
                <div className="space-y-1">
                  <span className="text-3xl lg:text-4xl font-light text-slate-900 tracking-tight block italic">
                    {stat.value}
                  </span>
                  <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-rose-400 transition-colors">
                    {stat.label}
                  </p>
                </div>
              </div>
      ...
      {/* --- CONTENT SECTIONS --- */}
      <section id="info-sections" className="py-24 bg-slate-50/40 px-10 lg:px-20">
        <div className="max-w-5xl mx-auto space-y-32">
          
          {/* What We Do */}
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl font-light text-slate-900 mb-6 italic font-serif">What We Do</h2>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 transition-all hover:shadow-md">
               <ul className="space-y-4">
                 {['42,000+ Unique Ecards', '4,000+ Categories', 'Global Celebration Hub'].map((item) => (
                   <li key={item} className="flex items-center gap-3 text-slate-600 font-light">
                     <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-rose-500"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>
                     {item}
                   </li>
                 ))}
               </ul>
          ...
          {/* Our Reach */}
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="order-2 md:order-1 relative rounded-[2rem] overflow-hidden h-64 shadow-inner bg-slate-200 group">
              <img src="https://images.unsplash.com/photo-1526772662000-3f88f10405ff?q=80&w=2000&auto=format&fit=crop" className="w-full h-full object-cover opacity-80 transition-transform duration-700 group-hover:scale-105" alt="Global Reach" />
            </div>
            <div className="order-1 md:order-2">
              <h2 className="text-3xl font-light text-slate-900 mb-6 italic font-serif">Our Reach</h2>
              <p className="text-xl text-slate-600 font-light leading-relaxed">
          ...
          {/* Business Model */}
          <div className="p-10 md:p-16 rounded-[3rem] bg-gradient-to-br from-slate-50 to-white border border-slate-200 shadow-xl relative overflow-hidden group">
            <div className="relative z-10">
              <h2 className="text-3xl font-light text-slate-900 mb-6 italic font-serif">Business Model</h2>
              <p className="text-xl text-slate-700 font-medium leading-relaxed max-w-2xl mb-8">
            ...
            <div className="absolute -bottom-10 -right-10 p-10 opacity-5 pointer-events-none text-slate-900 transition-transform duration-700 group-hover:scale-110">
